 ...
 */
template <typename Dtype>
class RangerSolver : public SGDSolver<Dtype> {
 public:
  explicit RangerSolver(const SolverParameter& param,
                      Solver<Dtype> *root_solver = NULL)
      : SGDSolver<Dtype>(param, root_solver) { RangerPreSolve();}
  explicit RangerSolver(const string& param_file,
                      Solver<Dtype> *root_solver = NULL)
      : SGDSolver<Dtype>(param_file, root_solver) { RangerPreSolve(); }
  virtual inline const char* type() const { return "Ranger"; }

 protected:
  void RangerPreSolve();
  virtual void ComputeUpdateValue(int param_id, Dtype rate);

  DISABLE_COPY_AND_ASSIGN(RangerSolver);
};
